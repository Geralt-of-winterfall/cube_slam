...
#include "detect_cuboid/detect_3d_cuboid.h"
#include "detect_cuboid/object_3d_util.h"
#include <algorithm>
...
namespace ORB_SLAM2
{
//构造函数，传入参数：system类，ORBVoc类，FrameDrawer类，MapDrawer类，Map类，KeyFrameDatabase类，sensor
...
{
    InitToGround = cv::Mat::eye(4, 4, CV_32F);//eye函数，4x4矩阵，CV_32F代表单精度浮点数类型，32F代表常量5，初始化为0矩阵。代表将相机初始位姿设置到地面坐标系上。
    ...
    for (int row = 0; row < 3; row++)
        for (int col = 0; col < 3; col++)
            InitToGround.at<float>(row, col) = rot(row, col);//构造变换矩阵旋转部分

    InitToGround.at<float>(0, 3) = init_x;
    InitToGround.at<float>(1, 3) = init_y;
    InitToGround.at<float>(2, 3) = init_z;//构造变换矩阵平移部分
    nominal_ground_height = init_z;//地面高度
    ...
    InitToGround_eigen = Converter::toMatrix4f(InitToGround);//初始相机位姿到
    ...
    if (whether_detect_object)
    {
        ...
        if (!whether_read_offline_cuboidtxt)//不读取离线数据
        {
            detect_cuboid_obj = new detect_3d_cuboid();//三维目标
            detect2d = new YOLOv5Detector();//YOLO二维目标检测
            detect_cuboid_obj->print_details = false; // false  true//true打印输出内容变多
            detect_cuboid_obj->whether_sample_cam_roll_pitch =
                ORB_SLAM2::otherSettings["whether_sample_cam_roll_pitch"];//是否三个旋转方向采样
        ...
        if (!whether_read_offline_cuboidtxt)
        {
            whether_save_online_detected_cuboids =
                ORB_SLAM2::otherSettings["whether_save_online_detected_cuboids"];//保存目标检测结果
            if (whether_save_online_detected_cuboids)
            {
                std::string save_object_pose_txt = data_folder + "/orb_live_pred_objs_temp.txt";//保存文件名
                save_online_detected_cuboids.open(save_object_pose_txt.c_str());
            }
        }
        if (whether_read_offline_cuboidtxt)
            ReadAllObjecttxt();//读取离线三维检测结果数据
    }

    if (whether_detect_object)
    {
        whether_save_final_optimized_cuboids =
            ORB_SLAM2::otherSettings["whether_save_final_optimized_cuboids"];//优化后最终三维目标检测结果
    ...
    Eigen::MatrixXd truth_cam_poses(5, 8); //真实位姿
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, int msg_seq_id)
{
    ...
    if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET)
    {
        ...
        { // read (truth) depth /stereo image for first frame
            ...
            // optimizer.
            std::string right_kitti_img_file = scene_folder + "/image_1/000000.png";
...
void Tracking::DetectCuboid(KeyFrame *pKF)
{
    ...
    else
    {
        ...
        //pop_pose_to_ground = pKF->GetPose();
        std::vector<detect_result> results;
        string k_detect_model_path = "/home/robertkidder/DeepSORT/build/yolov5s.onnx";
        ...
        std::cout << "检测目标数量：" << results.size() << endl;
        ...
        //Eigen::MatrixXd raw_all_obj2d_bbox(10, 5);//2d检测文件格式
        //Eigen::Matrix<double,10,5> raw_all_obj2d_bbox;
        std::vector<string> object_classes;//检测目标名称
        std::string txt_file_name = data_yolo_obj_dir + frame_index_c + ".txt";//std::cout << txt_file_name << std::endl;
        ...
        //read_2d_obj_txt(txt_file_name, raw_all_obj2d_bbox, object_classes);
        // if (!read_obj_detection_txt(txt_file_name, raw_all_obj2d_bbox, object_classes))
        // {
        ...
        // else
        {
            ...
            // }
            Eigen::MatrixXd all_obj2d_bbox_infov_mat(good_object_ids.size(), 5);
            //std::vector<Vect_inv> all_obj2d_bbox_infov_mat;
            for (size_t i = 0; i < good_object_ids.size(); i++)
            {
                Vector4d vec_bbox;
                for (int j=0;j<4;j++)
